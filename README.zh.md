...
- **恢复中心单一入口** — 设置 → 桌面一键打开壳原生恢复中心：版本清单与一键切换（npm/git 双来源）、插件救火（禁用/卸载/更新）、崩溃诊断集中一页；dsh 启动失败或意外退出时壳也会自动切入
...
- **设置页桌面分区** — 设置页新增「桌面」标签页（UI 契合 harness 设计）：后端来源卡片（来源模式、目录校验、克隆/准备环境、网络代理）、恢复中心入口卡片（打开壳原生恢复页：版本切换、插件救火、崩溃诊断）、桌壳自身更新检查、开机自启开关、启动最小化开关、端口策略、关于卡片
