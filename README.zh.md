...
dsh 迭代快、偶有破坏性变更，而插件的适配往往滞后——一次 dsh 更新就可能让后端起不来，昨天还好好的插件今天能把整个 harness 拖垮。在此之前，桌面壳能告诉你的只有一个死胡同式的「后端进程意外退出」弹窗：不知道是哪个插件、看不到具体报错、更没法就地处理。

恢复中心就是这个问题的答案——一个把"修好后端"变成几次点击的原生页面：

- **故障自动进入** — dsh 崩溃或启动失败时，壳自动切到恢复中心：滚动输出快照（token 已脱敏）+ 自动诊断，dsh 点名了插件就直接定位肇事者，认不出就诚实地给你完整日志
- **插件救火** — 禁用/启用写 profile 补丁层，写入前整体校验 + 原子替换（绝不破坏你手工编辑的行，也绝不会把后端写得起不来）；卸载/更新走 `dsh plugin` 命令并实时回显日志。宿主关键模块受保护名单约束，拒绝被禁用
- **版本切换，升级回退一体** — npm 全局与 git 源码双来源共用一份清单（渠道徽标、当前版本置顶），实时进度，完成后自动重启回正常页面
- **单一入口** — 一切正常时它也是版本维护的家：设置 → 桌面检查更新，确认后交接给恢复中心执行（没有吓人的诊断观感，只有进度）——全程无需命令行
...
- **源码渠道更新同样无需终端** — 壳核对上游 tag，一键完成检出指定版本、重装依赖、重新构建并重启后端——与恢复中心驱动的是同一条管线，只是入口在设置页；工作区有未提交修改时拒绝并明确提示
...
- **设置页桌面分区** — 设置页新增「桌面」标签页（UI 契合 harness 设计）：后端来源卡片（来源模式、目录校验、克隆/准备环境、网络代理）、dsh 版本卡片（按来源检查 → 更新交接给恢复中心）、桌壳自身更新检查、开机自启开关、启动最小化开关、端口策略、关于卡片
...
- **dsh**：壳从你所选的来源运行后端 — npm 全局（PATH + `npm root -g` 定位，可在设置 → 桌面一键升级）或本地检出（启动前校验 manifest、`node_modules/tsx`、已构建的前端 dist，dsh ≥ 0.1.3 还会校验 `fs-ext` 依赖）

## 项目结构

```
src/
  main.ts               应用生命周期：单实例锁、窗口、托盘、后端解析、恢复模式 IPC、引导页
  paths.ts              dev/prod 资源路径解析（图标、preload、桌面插件 patch、恢复页）
  ...
  dsh-source.ts         git 源码来源：目录校验（manifest/tsx/web dist，dsh ≥ 0.1.3 的 fs-ext 门控）、tag 解析、启动参数
  ...
  recovery/             恢复中心核心：崩溃诊断、profile 补丁层、插件清单与开关、恢复状态、主题匹配、IPC 来源校验
  kill-tree.ts          维护命令超时时的子进程树终止
